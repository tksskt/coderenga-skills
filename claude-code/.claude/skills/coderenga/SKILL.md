...
# CodeRenga Skill for Claude Code
...
CodeRenga is a local CLI coding agent. It can run in modes such as `coder`, `debug`, `architect`, and `reviewer`. Claude Code may also operate it as a subprocess worker for scoped implementation, debugging, review, architecture, or mode-policy verification tasks.
...
```bash
bash .claude/skills/coderenga/scripts/install-coderenga.sh
```
...
Delegated implementation from Claude Code:
...
Use `--non-interactive` when Claude Code calls CodeRenga as a child worker. This prevents confirmation prompts from hanging the parent agent.
