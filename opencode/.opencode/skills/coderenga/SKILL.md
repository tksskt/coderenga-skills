...
CodeRenga is a local CLI coding agent. It can run in modes such as `coder`, `debug`, `architect`, and `reviewer`. OpenCode may also operate it as a subprocess worker for scoped implementation, debugging, review, architecture, or mode-policy verification tasks.

The optional `AGENTS.md` bridge can route more natural-language requests toward this skill. Keep the detailed workflow here and use `AGENTS.md` only as a small bridge, especially when merging into a project that already has an `AGENTS.md`.
...
Delegated implementation from OpenCode:
...
Use `--non-interactive` when OpenCode calls CodeRenga as a child worker. This prevents confirmation prompts from hanging the parent agent.
